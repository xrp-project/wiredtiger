  ...
  uint8_t *data_buf, uint8_t *scratch_buf, struct wt_btree_scratch **resp)
{
    ...
    /*
     * bpfof fabric read: the traversal runs on the target (pinned program),
     * so there is no per-call BPF fd and no host data buffer; the scratch
     * carries the key in and the result out. fds is a one-element array.
     */
    {
        unsigned int fds_arr[10];
        fds_arr[0] = (unsigned int)fd;
        (void)bpf_fd;
        (void)data_buf;
        ret = syscall(SYS_READ_BPFOF, fds_arr, (size_t)1, (size_t)BTREE_BLOCK_SIZE,
          (off_t)root_offset, scratch_buf, (size_t)BUFFER_SIZE);
    }
    if (ret < 0) {
        fprintf(stderr, "read_bpfof: ret %ld errno %d\n", ret, errno);
        return (-1);
    }
    *resp = scratch;
    return (0);
...
main(int argc, char **argv)
{
    ...
    /* bpfof: the program is pinned on the target, nothing to load here. */
    (void)obj;
    bpf_fd = -1;
